...
&emsp;&emsp; 本次评测主要采用人工评测方式。在评估参赛团队的手语数字人时，评测人员会选择特定场景，输入一段文字，生成对应的视频，即数字人的翻译已完成。人工评测用于综合评估手语数字人在各指标方面的表现，每项指标的总得分为去掉一个最高分和一个最低分后的所有评分的算术平均值。假设有一组评分 X={x_1,x_2,…,x_n }，具体的计算公式如下：
